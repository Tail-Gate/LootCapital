...
#
#SBATCH --job-name=xgboost_walk    # Job name that appears in squeue
...
#SBATCH --output=xgboost_walk_log_%j.txt     # Standard output file, %j is replaced by job number
...
echo "Starting memory-optimized xgboost_walk script..."
...
echo "  - Using CPU-only training for XGBoost optimization"
echo "  - Using all 32 cores for parallel processing"
echo "  - No GPUs requested (XGBoost runs efficiently on CPU)"
echo "  - Using 120GB memory (maximum available)"

python XGBoostMean/xgboost_walk_forward_optimizer.py
